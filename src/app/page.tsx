      ...
      <section id="pricing" style={{ padding: '80px 48px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
          {[
            ...
            { name: 'Enterprise', price: 'Custom', period: '', features: ['Everything in Pro', 'SSO / SAML', 'Dedicated support', 'SLA guarantee', 'Custom integrations'], cta: 'Contact Sales', href: '/contact' },
          ].map((p, i) => (
            <div key={i} style={{ padding: '32px', background: p.featured ? '#3b82f610' : '#111', borderRadius: '12px', border: `1px solid ${p.featured ? '#3b82f640' : '#222'}`, position: 'relative' }}>
              ...
              <a href={p.href} style={{ display: 'block', textAlign: 'center', padding: '10px', background: p.featured ? '#3b82f6' : '#1a1a1a', color: '#fff', borderRadius: '8px', textDecoration: 'none', fontWeight: 500, fontSize: '14px', border: p.featured ? 'none' : '1px solid #333' }}>{p.cta}</a>
            </div>
          ))}
        </div>
      </section>
